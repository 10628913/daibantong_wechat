...
class Service
{
     ...
     */
    public static function refundOrder($orderdata, $type, $config = [])
    {
     ...
     * @return mixed|string
     */
    public static function getWechatMiniOpenid($custom = [])
    {
        $openid = '';
        $auth = Auth::instance();
        if ($auth->isLogin()) {
            $third = get_addon_info('third');
            if ($third && $third['state']) {
                $thirdInfo = Third::where('user_id', $auth->id)->where('platform', 'wechatmini')->find();
                $openid = $thirdInfo ? $thirdInfo['openid'] : '';
            }
        }
        if (!$openid) {
